...
2. **대기열 SSE** — 1,000명 대기 → 3초마다 빈 자리만큼 원자 승급(동시 active 100명 상한), 순번이 **실시간 감소** → 입장 (AUTO/FORCE_ON 토글) → [§3.4](#34-대기열-흐름-sse--polling-fallback)

3. **부하 회복** — 1,000 동접 → Grafana에서 **에러율 0% · 예약 p95 14ms · HikariCP 안정** / Zipkin에 6개 서비스 전 구간 계측 → [§5.3](#53-테스트-결과)
...
| 2 | **트래픽 폭증** — 인기 노선 오픈 시 DB 커넥션 풀 고갈 | Redis Sorted Set 대기열 + 자리 기반(capacity-aware) 승급 — 동시 active 100명 상한 |
...
- **기간**: 2026.05 ~ 2026.07 (약 2개월, M0~M9 완료)
...
| 서비스 | 포트 | 책임 | 저장소 |
|--------|------|------|--------|
| **api-gateway** | 8080 | JWT 검증, `X-User-*` 헤더 주입, CORS, Redis 고정 윈도우 Rate Limit, CAPTCHA, SSE 라우팅 | — |
...
sequenceDiagram
    ...
    Note over Q: @Scheduled(fixedDelay=3s)<br/>promote.lua — 빈 자리만큼만 원자 승급<br/>(max-active 100, batch-size 100)
    Q->>RD: ZADD queue:active:set:global expiresAt(+600s) {userId}
...
**대기열 SSE 실시간 순번 감소** — 1,000명 대기 → 3초마다 빈 자리만큼 승급(active 100명 상한) → 입장
...
- `payment-service`: Transactional Outbox(`OutboxRelayScheduler`) + Redisson lock + JPA `@Version` 낙관락 + DLT
...
| Rate Limiting | Redis Lua 고정 윈도우 (+ 인메모리 fallback) |
